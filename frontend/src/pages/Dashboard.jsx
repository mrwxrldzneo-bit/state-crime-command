...
export default function Dashboard() {
  ...
  const openCaseDetail = async (caseData, options = {}) => {
    setFollowUpRecommendations([]);
    setWorkspaceError("");
    setWorkspaceTab("overview");
    if (!options.keepHubOpen) setWorkspaceHubOpen(false);
    setSelectedCase(caseData);
    primeCommandReview(caseData);
    setCaseNote("");
    setCaseNoteError("");
    if (!options.keepDetailClosed) setIsDetailOpen(true);
                  ...
                  <div className="sticky top-0 z-20 -mx-4 md:-mx-5 -mt-4 md:-mt-5 mb-4 px-4 md:px-5 py-4 border-b border-[#294766] bg-[#081726]/[0.995] backdrop-blur-2xl flex items-center justify-between gap-4">
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-col sm:flex-row sm:items-center gap-2">
                        <select
                          value={getCaseKey(selectedCase) || ""}
                          onChange={(event) => {
                            const nextCase = cases.find((item) => getCaseKey(item) === event.target.value);
                            if (nextCase) {
                              openCaseDetail(nextCase, { keepDetailClosed: true, keepHubOpen: true });
                              setWorkspaceHubOpen(true);
                            }
                          }}
                          className={`${INPUT_CLASS} sm:max-w-[430px]`}
                        >
                          {cases
                            .filter((item) => ["opened", "closed"].includes(String(item.status || "").toLowerCase()) || item.workspace_unlocked)
                            .map((item) => (
                              <option key={getCaseKey(item)} value={getCaseKey(item)}>
                                {item.case_id} · {item.name}
                              </option>
                            ))}
                        </select>
                        <span className="text-[10px] font-mono uppercase tracking-wider text-[#607793]">Selected Case File</span>
                      </div>
                      <p className="mt-2 text-[10px] text-[#7186a0]">Investigation tools, AI interviews, legislation, briefings and case activity in one place.</p>
                    </div>
      ...
      <button
        ...
        onClick={() => {
          const eligibleCases = cases.filter(
            (item) =>
              ["opened", "closed"].includes(String(item.status || "").toLowerCase()) ||
              item.workspace_unlocked
          );
          if (!eligibleCases.length) {
            setWorkspaceError("No approved Case Files are available yet.");
            setWorkspaceHubOpen(true);
            return;
          }
          if (
            !selectedCase ||
            !eligibleCases.some((item) => getCaseKey(item) === getCaseKey(selectedCase))
          ) {
            openCaseDetail(eligibleCases[0], { keepDetailClosed: true, keepHubOpen: true });
          }
          setWorkspaceHubOpen((open) => !open);
        }}
        className="fixed right-5 md:right-7 bottom-5 z-[85] min-w-[220px] px-5 py-3.5 rounded-2xl border border-[#d4b25a]/70 bg-[#0b1d31] shadow-[0_18px_55px_rgba(0,0,0,0.68)] text-left hover:bg-[#102840] transition"
      >
        <span className="flex items-center gap-3">
          <BrainCircuit className="h-5 w-5 text-[#d4b25a]"/>
          <span>
            <span className="block text-[10px] font-mono uppercase tracking-[0.18em] text-[#d4b25a]">SCC Investigation Hub</span>
            <span className="block mt-0.5 text-xs font-semibold text-white">{workspaceHubOpen ? "Close investigation tools" : "Open investigation tools"}</span>
          </span>
        </span>
      </button>
