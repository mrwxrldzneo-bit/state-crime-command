...
import { Shield, LogOut, Clock, FolderOpen, Archive, Search, Eye, Edit2, Trash2 } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-[#061326] text-[#e7edf6] font-sans antialiased relative scc-scanlines scc-vignette">
      ...
      <header className="bg-[#0b1b33] border-b border-[#1c3557] px-6 py-4 flex items-center justify-between shadow-md">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            <p className="text-xs font-mono text-[#8ba0bd] uppercase">Agent: {user?.username || "admin"}</p>
            <p className="text-[10px] font-display uppercase tracking-wider text-[#d4b25a]">{user?.role || "administrator"}</p>
          </div>
          <button onClick={logout} className="p-2 hover:bg-[#1c3557] rounded text-[#f1a8a8] transition-colors cursor-pointer bg-transparent border-none">
      ...
      <main className="max-w-7xl mx-auto p-6 flex flex-col gap-6">
        ...
        {/* Filters and Search Bar Module Grid Container */}
                ...
                <tr key={c.id || Math.random()} className="hover:bg-[#102540]/30 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right font-sans">
                    <div className="flex gap-2 justify-end">
                      <button className="p-1 hover:bg-[#1c3557] rounded text-[#8ba0bd] transition-colors cursor-pointer bg-transparent border-none">
                        <Eye className="h-3.5 w-3.5" />
                      </button>
                      <button className="p-1 hover:bg-[#1c3557] rounded text-blue-400 transition-colors cursor-pointer bg-transparent border-none">
                        <Edit2 className="h-3.5 w-3.5" />
                      </button>
                      <button className="p-1 hover:bg-[#1c3557] rounded text-rose-400 transition-colors cursor-pointer bg-transparent border-none">
