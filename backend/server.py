...
SCC_AI_API_URL = os.environ.get("SCC_AI_API_URL", "").strip().rstrip("/")
SCC_AI_API_KEY = os.environ.get("SCC_AI_API_KEY", "").strip()
SCC_AI_MODEL = os.environ.get("SCC_AI_MODEL", "").strip()
...
class InformationGapCreate(BaseModel):
    question: str
    detail: str = ""
    priority: str = "medium"
...
class InterviewPrepareRequest(BaseModel):
    subject_name: str
    interview_type: str = "Suspect / Person of Interest"
    officer_rank: str
    officer_name: str
    second_officer_present: bool = False
    second_officer_rank: str = ""
    second_officer_name: str = ""
    circumstance: str = ""


class InterviewLiveTurnRequest(BaseModel):
    transcript: str
...
class OperationDebriefRequest(BaseModel):
    outcome: str
...
# ---------------------------------------------------------------------------
# SCC Intelligence helpers
# ---------------------------------------------------------------------------

def _case_suffix(doc: dict) -> str:
    return str(doc.get("case_id") or "SCC").split("-")[-1]

def _next_record_id(doc: dict, field: str, prefix: str) -> str:
    ...
    return f"{prefix}-{_case_suffix(doc)}-{max(numbers or [0]) + 1:03d}"

def build_case_context(doc: dict) -> dict:
    return {
        "case": {
            "case_id": doc.get("case_id"), "name": doc.get("name"), "division": doc.get("division"),
            "priority": doc.get("priority"), "status": doc.get("status"), "lead_investigator": doc.get("lead_investigator"),
            "request_basis": doc.get("request_basis"), "known_information": doc.get("known_information"),
            "investigation_objective": doc.get("investigation_objective"), "case_file": doc.get("case_file"),
        },
        "investigation_log": doc.get("investigation_log") or [],
        "evidence": [{k: v for k, v in item.items() if k != "file_id"} for item in (doc.get("evidence") or []) if not item.get("removed")],
        "information_gaps": doc.get("information_gaps") or [],
...
async def create_notification(username: str, title: str, message: str, case_uid: str = "", kind: str = "info"):
    if not username:
        return
...
async def _call_scc_ai(system_prompt: str, user_payload: dict) -> Optional[dict]:
    if not (SCC_AI_API_URL and SCC_AI_API_KEY and SCC_AI_MODEL):
        return None
    url = SCC_AI_API_URL if SCC_AI_API_URL.endswith("/chat/completions") else f"{SCC_AI_API_URL}/chat/completions"
    headers = {"Authorization": f"Bearer {SCC_AI_API_KEY}", "Content-Type": "application/json"}
    payload = {"model": SCC_AI_MODEL, "temperature": 0.2, "response_format": {"type": "json_object"}, "messages": [{"role": "system", "content": system_prompt}, {"role": "user", "content": json.dumps(user_payload, ensure_ascii=False)}]}
    try:
        async with httpx.AsyncClient(timeout=75) as hc:
            res = await hc.post(url, headers=headers, json=payload)
        if not res.is_success:
            logger.warning("SCC AI request failed HTTP %s: %s", res.status_code, res.text[:500])
            return None
        data = res.json()
        content = data.get("choices", [{}])[0].get("message", {}).get("content", "")
        return json.loads(content) if content else None
    except Exception as exc:
        logger.warning("SCC AI unavailable: %s", exc)
        return None

def _fallback_interview_opening(req: InterviewPrepareRequest) -> str:
    ts = datetime.now(timezone.utc)
    presence = ""
    if req.second_officer_present and req.second_officer_name.strip():
        presence = f", and also present is {req.second_officer_rank.strip()} {req.second_officer_name.strip()}"
    lines = [f"This interview commenced at {ts.strftime('%H:%M')} hours on {ts.strftime('%d %B %Y')}.", f"For the record, I am {req.officer_rank.strip()} {req.officer_name.strip()} of the State Crime Command{presence}.", f"The person being interviewed today is {req.subject_name.strip()}. {req.subject_name.strip()}, could you please state your full name for the record?"]
    if req.circumstance.strip():
        lines.append(req.circumstance.strip())
    lines.append("Before we proceed, I will explain the interview process and any applicable caution or participation requirements for this interview type.")
    return "\n\n".join(lines)

def _fallback_interview_objectives(doc: dict) -> List[dict]:
    objectives = []
    if doc.get("investigation_objective"):
        objectives.append({"id": "OBJ-001", "label": doc.get("investigation_objective"), "status": "unresolved"})
    for gap in (doc.get("information_gaps") or []):
        if str(gap.get("status", "open")).lower() != "resolved":
            objectives.append({"id": gap.get("id"), "label": gap.get("question"), "status": "unresolved"})
    return objectives[:8] or [{"id": "OBJ-001", "label": "Obtain the subject's complete account relevant to the case objective.", "status": "unresolved"}]

def _fallback_next_actions(doc: dict) -> List[dict]:
    actions = []
    for gap in (doc.get("information_gaps") or []):
        if str(gap.get("status", "open")).lower() != "resolved":
            actions.append({"priority": "high" if gap.get("priority") == "high" else "medium", "title": f"Progress {gap.get('id', 'information gap')}", "why": gap.get("question") or "An information gap remains unresolved.", "action_type": "gap", "linked_record": gap.get("id", "")})
    for task in (doc.get("tasks") or []):
        if str(task.get("status", "open")).lower() != "completed":
            actions.append({"priority": task.get("priority", "routine"), "title": task.get("title") or "Complete outstanding task", "why": "This task remains outstanding on the active case.", "action_type": "task", "linked_record": task.get("id", "")})
    if not (doc.get("interviews") or []):
        actions.append({"priority": "medium", "title": "Consider a case interview", "why": "No completed interview is currently recorded against this active case.", "action_type": "interview", "linked_record": ""})
    return actions[:5] or [{"priority": "routine", "title": "Review the case objective", "why": "No specific outstanding intelligence item was detected. Review the objective and latest investigation log before the next action.", "action_type": "review", "linked_record": ""}]
...
async def get_evidence_file(case_uid: str, evidence_id: str, user: dict = Depends(get_current_user)):
    ...
    stream = io.BytesIO()
    try:
        await get_evidence_fs().download_to_stream(ObjectId(item["file_id"]), stream)
    except Exception:
        raise HTTPException(status_code=404, detail="Evidence file not found.")
...
# ---------------------------------------------------------------------------
# SCC Intelligence / progression endpoints
# ---------------------------------------------------------------------------

@api_router.post("/cases/{case_uid}/gaps", response_model=Case)
async def create_information_gap(case_uid: str, body: InformationGapCreate, user: dict = Depends(get_current_user)):
    doc = await _workspace_case(case_uid)
    question = body.question.strip()
    if not question: raise HTTPException(status_code=400, detail="Information gap question is required.")
    item = {"id": _next_record_id(doc, "information_gaps", "GAP"), "question": question, "detail": body.detail.strip(), "priority": body.priority.strip().lower() or "medium", "status": "open", "created_by": user["username"], "created_at": now_iso(), "resolution": ""}
    await db.cases.update_one({"id": case_uid}, {"$push": {"information_gaps": item, "activity_log": _activity(user["username"], "INFORMATION GAP CREATED", f"{item['id']} · {question}")}, "$set": {"updated_at": now_iso()}})
    return await db.cases.find_one({"id": case_uid}, {"_id": 0})

@api_router.put("/cases/{case_uid}/gaps/{gap_id}", response_model=Case)
async def update_information_gap(case_uid: str, gap_id: str, body: InformationGapUpdate, user: dict = Depends(get_current_user)):
    await _workspace_case(case_uid)
    allowed = {"open", "partially addressed", "resolved"}
    updates = {"updated_at": now_iso(), "information_gaps.$.updated_by": user["username"], "information_gaps.$.updated_at": now_iso()}
    if body.status is not None:
        status = body.status.strip().lower()
        if status not in allowed: raise HTTPException(status_code=400, detail="Invalid information gap status.")
        updates["information_gaps.$.status"] = status
    if body.detail is not None: updates["information_gaps.$.detail"] = body.detail.strip()
    if body.resolution is not None: updates["information_gaps.$.resolution"] = body.resolution.strip()
    if body.priority is not None: updates["information_gaps.$.priority"] = body.priority.strip().lower()
    result = await db.cases.update_one({"id": case_uid, "information_gaps.id": gap_id}, {"$set": updates, "$push": {"activity_log": _activity(user["username"], "INFORMATION GAP UPDATED", gap_id)}})
    if not result.matched_count: raise HTTPException(status_code=404, detail="Information gap not found.")
    return await db.cases.find_one({"id": case_uid}, {"_id": 0})

@api_router.post("/cases/{case_uid}/leads", response_model=Case)
async def create_lead(case_uid: str, body: LeadCreate, user: dict = Depends(get_current_user)):
    doc = await _workspace_case(case_uid)
    if not body.title.strip(): raise HTTPException(status_code=400, detail="Lead title is required.")
    item = {"id": _next_record_id(doc, "leads", "LEAD"), "title": body.title.strip(), "detail": body.detail.strip(), "linked_gap_id": body.linked_gap_id.strip(), "priority": body.priority.strip().lower() or "medium", "status": "new", "created_by": user["username"], "created_at": now_iso()}
    await db.cases.update_one({"id": case_uid}, {"$push": {"leads": item, "activity_log": _activity(user["username"], "INVESTIGATION LEAD CREATED", f"{item['id']} · {item['title']}")}, "$set": {"updated_at": now_iso()}})
    return await db.cases.find_one({"id": case_uid}, {"_id": 0})

@api_router.put("/cases/{case_uid}/leads/{lead_id}", response_model=Case)
async def update_lead(case_uid: str, lead_id: str, body: LeadUpdate, user: dict = Depends(get_current_user)):
    await _workspace_case(case_uid)
    allowed = {"new", "developing", "verified", "disproved", "closed"}
    updates = {"updated_at": now_iso(), "leads.$.updated_by": user["username"], "leads.$.updated_at": now_iso()}
    if body.status is not None:
        status = body.status.strip().lower()
        if status not in allowed: raise HTTPException(status_code=400, detail="Invalid lead status.")
        updates["leads.$.status"] = status
    if body.detail is not None: updates["leads.$.detail"] = body.detail.strip()
    if body.priority is not None: updates["leads.$.priority"] = body.priority.strip().lower()
    result = await db.cases.update_one({"id": case_uid, "leads.id": lead_id}, {"$set": updates, "$push": {"activity_log": _activity(user["username"], "INVESTIGATION LEAD UPDATED", lead_id)}})
    if not result.matched_count: raise HTTPException(status_code=404, detail="Lead not found.")
    return await db.cases.find_one({"id": case_uid}, {"_id": 0})

@api_router.post("/cases/{case_uid}/tasks", response_model=Case)
async def create_case_task(case_uid: str, body: TaskCreate, user: dict = Depends(get_current_user)):
    doc = await _workspace_case(case_uid)
    if not body.title.strip(): raise HTTPException(status_code=400, detail="Task title is required.")
    item = {"id": _next_record_id(doc, "tasks", "TASK"), "title": body.title.strip(), "detail": body.detail.strip(), "assigned_to": body.assigned_to.strip(), "priority": body.priority.strip().lower() or "routine", "due_at": body.due_at.strip(), "linked_record": body.linked_record.strip(), "status": "open", "created_by": user["username"], "created_at": now_iso()}
    await db.cases.update_one({"id": case_uid}, {"$push": {"tasks": item, "activity_log": _activity(user["username"], "TASK CREATED", f"{item['id']} · {item['title']}")}, "$set": {"updated_at": now_iso()}})
    if item["assigned_to"]: await create_notification(item["assigned_to"], f"Task assigned · {item['id']}", item["title"], case_uid, "task")
    return await db.cases.find_one({"id": case_uid}, {"_id": 0})

@api_router.put("/cases/{case_uid}/tasks/{task_id}", response_model=Case)
async def update_case_task(case_uid: str, task_id: str, body: TaskUpdate, user: dict = Depends(get_current_user)):
    await _workspace_case(case_uid)
    allowed = {"open", "in progress", "completed"}
    updates = {"updated_at": now_iso(), "tasks.$.updated_by": user["username"], "tasks.$.updated_at": now_iso()}
    if body.status is not None:
        status = body.status.strip().lower()
        if status not in allowed: raise HTTPException(status_code=400, detail="Invalid task status.")
        updates["tasks.$.status"] = status
        if status == "completed": updates["tasks.$.completed_at"] = now_iso()
    if body.detail is not None: updates["tasks.$.detail"] = body.detail.strip()
    if body.assigned_to is not None: updates["tasks.$.assigned_to"] = body.assigned_to.strip()
    if body.priority is not None: updates["tasks.$.priority"] = body.priority.strip().lower()
    if body.due_at is not None: updates["tasks.$.due_at"] = body.due_at.strip()
    result = await db.cases.update_one({"id": case_uid, "tasks.id": task_id}, {"$set": updates, "$push": {"activity_log": _activity(user["username"], "TASK UPDATED", task_id)}})
    if not result.matched_count: raise HTTPException(status_code=404, detail="Task not found.")
    return await db.cases.find_one({"id": case_uid}, {"_id": 0})

@api_router.post("/cases/{case_uid}/ai/next-actions")
async def case_next_actions(case_uid: str, body: FollowUpRequest, user: dict = Depends(get_current_user)):
    doc = await _workspace_case(case_uid)
    ai = await _call_scc_ai("You are SCC Case Intelligence. Return strict JSON with key recommendations, an array of 3-5 objects containing priority, title, why, action_type, linked_record. Use only supplied case information; never invent facts or legislation.", {"task": "Recommend next investigative actions", "trigger": body.trigger, "context": build_case_context(doc)})
    recommendations = (ai or {}).get("recommendations") if isinstance(ai, dict) else None
    if not isinstance(recommendations, list): recommendations = _fallback_next_actions(doc)
    item = {"id": str(uuid.uuid4()), "trigger": body.trigger, "recommendations": recommendations[:5], "created_by": user["username"], "created_at": now_iso()}
    await db.cases.update_one({"id": case_uid}, {"$push": {"ai_recommendations": item}, "$set": {"updated_at": now_iso()}})
    return item

@api_router.post("/cases/{case_uid}/interviews/prepare")
async def prepare_interview(case_uid: str, body: InterviewPrepareRequest, user: dict = Depends(get_current_user)):
    doc = await _workspace_case(case_uid)
    if not body.subject_name.strip() or not body.officer_rank.strip() or not body.officer_name.strip(): raise HTTPException(status_code=400, detail="Subject, interviewing officer rank and name are required.")
    ai = await _call_scc_ai("You are SCC Interview Director for a fictional ER:LC investigation system. Return strict JSON with opening, objectives, first_question, script. Use only supplied case information. Distinguish established information from claims. Do not invent evidence, people, offences or legislation.", {"task": "Prepare a grounded interview", "setup": body.model_dump(), "context": build_case_context(doc)})
    opening = (ai or {}).get("opening") if isinstance(ai, dict) else None
    objectives = (ai or {}).get("objectives") if isinstance(ai, dict) else None
    first_question = (ai or {}).get("first_question") if isinstance(ai, dict) else None
    script = (ai or {}).get("script") if isinstance(ai, dict) else None
    if not opening: opening = _fallback_interview_opening(body)
    if not isinstance(objectives, list): objectives = _fallback_interview_objectives(doc)
    if not first_question: first_question = "Please tell me, in your own words, what you know about the circumstances relevant to this investigation."
    if not isinstance(script, list): script = [first_question]
    interview_id = f"INT-{_case_suffix(doc)}-{len(doc.get('interviews') or []) + 1:03d}"
    item = {"id": interview_id, "subject_name": body.subject_name.strip(), "interview_type": body.interview_type.strip(), "officer_rank": body.officer_rank.strip(), "officer_name": body.officer_name.strip(), "second_officer_present": body.second_officer_present, "second_officer_rank": body.second_officer_rank.strip(), "second_officer_name": body.second_officer_name.strip(), "circumstance": body.circumstance.strip(), "opening": opening, "objectives": objectives, "script": script, "next_question": first_question, "status": "prepared", "transcript": "", "created_by": user["username"], "created_at": now_iso()}
    await db.cases.update_one({"id": case_uid}, {"$push": {"interviews": item, "activity_log": _activity(user["username"], "INTERVIEW PREPARED", f"{interview_id} · {item['subject_name']}")}, "$set": {"updated_at": now_iso()}})
    return item

@api_router.post("/cases/{case_uid}/interviews/{interview_id}/live-turn")
async def interview_live_turn(case_uid: str, interview_id: str, body: InterviewLiveTurnRequest, user: dict = Depends(get_current_user)):
    doc = await _workspace_case(case_uid)
    interview = next((x for x in (doc.get("interviews") or []) if x.get("id") == interview_id), None)
    if not interview: raise HTTPException(status_code=404, detail="Interview not found.")
    ai = await _call_scc_ai("You are SCC Live Interview Copilot. Return strict JSON with next_question, why, alternate_questions, observations, objective_updates. Use only case context and transcript. Never presume guilt. Prefer one concise natural spoken question at a time.", {"interview": interview, "transcript": body.transcript[-12000:], "current_question": body.current_question, "manual_direction": body.manual_direction, "context": build_case_context(doc)})
    if not isinstance(ai, dict) or not ai.get("next_question"):
        unresolved = next((x for x in (doc.get("information_gaps") or []) if str(x.get("status", "open")).lower() != "resolved"), None)
        ai = {"next_question": f"I want to clarify one point: {unresolved.get('question')}" if unresolved else "What happened next, and who else was involved?", "why": "This keeps the interview focused on unresolved case information.", "alternate_questions": ["Can you be more specific about the timing?", "Who else can confirm that account?"], "observations": [], "objective_updates": []}
    await db.cases.update_one({"id": case_uid, "interviews.id": interview_id}, {"$set": {"interviews.$.status": "in_progress", "interviews.$.transcript": body.transcript, "interviews.$.next_question": ai.get("next_question", ""), "interviews.$.last_live_analysis": ai, "interviews.$.updated_at": now_iso(), "updated_at": now_iso()}})
    return ai

@api_router.post("/cases/{case_uid}/interviews/{interview_id}/recording")
async def save_interview_recording(case_uid: str, interview_id: str, request: Request, user: dict = Depends(get_current_user)):
    doc = await _workspace_case(case_uid)
    interview = next((x for x in (doc.get("interviews") or []) if x.get("id") == interview_id), None)
    if not interview: raise HTTPException(status_code=404, detail="Interview not found.")
    raw = await request.body()
    if not raw: raise HTTPException(status_code=400, detail="Recording is empty.")
    if len(raw) > 100 * 1024 * 1024: raise HTTPException(status_code=413, detail="Interview recording exceeds 100MB.")
    content_type = request.headers.get("content-type", "audio/webm").split(";")[0]
    file_id = await get_evidence_fs().upload_from_stream(f"{interview_id}.webm", raw, metadata={"case_uid": case_uid, "interview_id": interview_id, "content_type": content_type, "kind": "interview_recording"})
    await db.cases.update_one({"id": case_uid, "interviews.id": interview_id}, {"$set": {"interviews.$.recording_file_id": str(file_id), "interviews.$.recording_content_type": content_type, "interviews.$.recording_size": len(raw), "updated_at": now_iso()}})
    return {"saved": True, "size": len(raw)}

@api_router.get("/cases/{case_uid}/interviews/{interview_id}/recording")
async def get_interview_recording(case_uid: str, interview_id: str, user: dict = Depends(get_current_user)):
    doc = await db.cases.find_one({"id": case_uid}, {"_id": 0, "interviews": 1})
    interview = next((x for x in (doc or {}).get("interviews", []) if x.get("id") == interview_id), None)
    if not interview or not interview.get("recording_file_id"): raise HTTPException(status_code=404, detail="Interview recording not found.")
    import io
    stream = io.BytesIO()
    await get_evidence_fs().download_to_stream(ObjectId(interview["recording_file_id"]), stream)
    return Response(stream.getvalue(), media_type=interview.get("recording_content_type") or "audio/webm", headers={"Content-Disposition": f'inline; filename="{interview_id}.webm"', "Cache-Control": "private, no-store"})

@api_router.post("/cases/{case_uid}/interviews/{interview_id}/finalize")
async def finalize_interview(case_uid: str, interview_id: str, body: InterviewFinalizeRequest, user: dict = Depends(get_current_user)):
    doc = await _workspace_case(case_uid)
    interview = next((x for x in (doc.get("interviews") or []) if x.get("id") == interview_id), None)
    if not interview: raise HTTPException(status_code=404, detail="Interview not found.")
    transcript = body.transcript.strip()
    ai = await _call_scc_ai("You are SCC Post-Interview Assessment. Return strict JSON with account_summary, material_new_information, admissions_denials_claims, potential_discrepancies, evidence_discussed, objectives, gaps, potential_legal_issues, recommended_outcomes. Never invent legislation. Legal issues require verification. Recommendations require human confirmation.", {"interview": interview, "transcript": transcript, "context": build_case_context(doc)})
    if not isinstance(ai, dict): ai = {"account_summary": transcript[-2500:] if transcript else "No transcript was captured.", "material_new_information": [], "admissions_denials_claims": [], "potential_discrepancies": [], "evidence_discussed": [], "objectives": interview.get("objectives") or [], "gaps": [], "potential_legal_issues": [], "recommended_outcomes": _fallback_next_actions(doc)}
    assessment = {**ai, "generated_at": now_iso(), "generated_by": "SCC Intelligence", "human_confirmation_required": True}
...
async def plan_operation(case_uid: str, body: OperationPlanRequest, user: dict = Depends(require_admin)):
    ...
    operation_id = f"OPR-{_case_suffix(doc)}-{len(doc.get('operations') or []) + 1:03d}"
    item = {"id": operation_id, "operation_type": body.operation_type.strip(), "command_instructions": body.command_instructions.strip(), "briefing": ai, "status": "planned", "created_by": user["username"], "created_at": now_iso()}
    await db.cases.update_one({"id": case_uid}, {"$push": {"operations": item, "activity_log": _activity(user["username"], "OPERATION PLANNED", f"{operation_id} · {body.operation_type}")}, "$set": {"updated_at": now_iso()}})
    return item
...
@api_router.post("/cases/{case_uid}/operations/{operation_id}/debrief", response_model=Case)
async def debrief_operation(case_uid: str, operation_id: str, body: OperationDebriefRequest, user: dict = Depends(get_current_user)):
    doc = await _workspace_case(case_uid)
    operation = next((x for x in (doc.get("operations") or []) if x.get("id") == operation_id), None)
    if not operation: raise HTTPException(status_code=404, detail="Operation not found.")
    ai = await _call_scc_ai("You are SCC Operation Debrief. Return strict JSON with summary, objectives_achieved, objectives_unresolved, new_information, evidence_obtained, remaining_gaps, potential_leads, follow_up_recommendations. Use only supplied outcome and case context.", {"operation": operation, "outcome": body.model_dump(), "context": build_case_context(doc)})
    if not isinstance(ai, dict): ai = {"summary": body.outcome, "objectives_achieved": [], "objectives_unresolved": [], "new_information": [], "evidence_obtained": body.evidence_obtained, "remaining_gaps": body.unresolved_matters, "potential_leads": [], "follow_up_recommendations": _fallback_next_actions(doc)}
    await db.cases.update_one({"id": case_uid, "operations.id": operation_id}, {"$set": {"operations.$.status": "completed", "operations.$.debrief": ai, "operations.$.completed_by": user["username"], "operations.$.completed_at": now_iso(), "updated_at": now_iso()}, "$push": {"activity_log": _activity(user["username"], "OPERATION COMPLETED", operation_id)}})
    return await db.cases.find_one({"id": case_uid}, {"_id": 0})
...
async def mark_notifications_read(body: NotificationReadRequest, user: dict = Depends(get_current_user)):
    query = {"username": user["username"]}
    if body.notification_ids: query["id"] = {"$in": body.notification_ids}
